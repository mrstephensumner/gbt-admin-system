...
- [x] T001 Add `talentAvailability` table + `working_week` column on `talent` to
...
- [x] T002 [P] Create `app/shared/availability.ts`: state keys/labels/tones, working-week options,
...
- [x] T003 [P] Unit tests `app/tests/unit/availability.test.ts`: state vocab (4 keys/tones),
  ...
  `isWorkingDay`.
- [x] T004 Create `app/worker/services/availability.ts`: `listMonth(reference, month)` (overlap
...
- [x] T005 [US1] Add `GET /api/talent/:reference/availability` route (month param) → `listMonth`;
  wire the group in `app/worker/index.ts`.
- [x] T006 [P] [US1] Add availability types to `app/src/lib/types.ts`.
- [x] T007 [US1] Build the calendar in `app/src/routes/availability-tab.tsx`: month header +
...
- [x] T008 [US1] Replace the Availability `coming-soon` block in
  `app/src/routes/talent-profile.tsx` with `<AvailabilityTab>`.
- [x] T009 [US1] Integration tests `app/tests/integration/availability.test.ts` (part 1): GET
...
- [x] T010 [US2] Add `POST` / `PATCH :id` / `DELETE :id` routes → service create/update/remove
  (validate state, title, `bad_range`).
- [x] T011 [US2] Add/edit dialog in `availability-tab.tsx` (state select, title, detail, location,
...
- [x] T012 [P] [US2] Integration tests (part 2): create/edit/remove round-trip; `bad_range` (end <
...
- [x] T013 [US3] Add the "This month" panel to `availability-tab.tsx` (date, title, detail ·
  location, state badge; date-ordered; derived from the same month entries).
- [x] T014 [P] [US3] Integration/unit coverage that the list source equals the calendar source for a
...
- [x] T015 [US4] Add `PATCH /api/talent/:reference/availability/settings` → `setWorkingWeek`
  (validate; `working_week_changed` record).
- [x] T016 [US4] Add the Sync panel to `availability-tab.tsx`: working-week `Select` (persists), and
  a **Connect Google Calendar** control clearly marked not-yet-active (disabled + "Coming soon").
- [x] T017 [P] [US4] Integration test (part 3): setting persists + `bad_working_week` refused.
...
- [x] T018 [US5] Ensure every service mutation writes a change record; add human-readable labels for
  `availability_added` / `availability_updated` / `availability_removed` / `working_week_changed` in
  the History tab and dashboard feed.
- [x] T019 [US5] Confirm `app/worker/services/serialize.ts` excludes availability + working_week
  (assert/guard).
- [x] T020 [P] [US5] Integration guard tests (part 4): publish-safe shape has no availability fields
  (SC-005); each mutation appears in History attributed (SC-006).

## Phase 8 — Polish & cross-cutting

- [x] T021 [P] e2e `app/tests/e2e/us-availability.spec.ts`: calendar renders with legend; Block
...
- [x] T022 [P] Update `CHANGELOG.md` + `docs/case-study.md` (note Google Calendar deferred).
- [x] T023 Full verification on a clean DB (unit + integration + e2e + lint + typecheck); migrate
